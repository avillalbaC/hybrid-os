...
function formatTrendValue(metric: TrendMetric, value: number | null) {
  ...
  if (metric.key.includes("RunMeters")) {
...
function TrendCard({ metric }: { metric: TrendMetric }) {
  ...
  return (
    <article className="rounded-md border border-[var(--line)] bg-[linear-gradient(180deg,var(--panel-strong),var(--panel))] p-4 shadow-[0_22px_70px_rgba(0,0,0,0.22)]">
      ...
      <p className="mt-3 text-sm leading-5 text-[var(--muted)]">{metric.message}</p>
    </article>
  );
}
    ...
    <section>
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-black tracking-tight">Progreso semanal</h3>
          <p className="mt-2 text-sm text-[var(--muted)]">Semana calendario lunes-domingo contra la media reciente.</p>
        </div>
        <Badge tone="accent">Últimas 4 semanas</Badge>
      </div>
      ...
      {!isLoading && trends.currentWeek ? (
        <div className="mt-4 rounded-md border border-[var(--line)] bg-[rgba(244,247,244,0.025)] p-4 text-sm leading-6 text-[var(--muted-strong)]">
          ...
          {formatTrendValue(trends.runExposure.total, trends.currentWeek.totalRunMeters)} total.
        </div>
      ) : null}
    </section>
  );
...
export function QuickTrendsCard({
  ...
  const quickTrends = [trends.runExposure.total, trends.load, trends.externalLoad.currentValue > 0 ? trends.externalLoad : trends.duration]
    .filter((trend, index, list) => list.findIndex((item) => item.key === trend.key) === index)
    .slice(0, 3);
